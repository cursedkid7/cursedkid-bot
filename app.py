...
class Store:
    ...
    def get_entry(self, entry_id):
        return self.data.get(str(entry_id), None)
    
    def mark_sold(self, entry_id, price):
        entry_id = str(entry_id)  # Ensure string
...
\uD83C\uDD94 ID: {entry_id}"""
    ...
    # Send confirmation separately with clear ID
...
def handle_sold(chat_id, args):
    ...
    if not entry:
        send_message(chat_id, f"\u274C Account ID {entry_id} not found.\n\nUse /list to see all available accounts with their IDs.")
        return
...
Available Accounts: {len(store.get_all())}"""
        ...
        send_message(chat_id, f"\u274C Failed to mark ID {entry_id} as sold. Please try again.")

def handle_list(chat_id):
    data = store.get_all()
    
    if not data:
        send_message(chat_id, f"\uD83D\uDCED No available credentials.\n\nTotal sold: {store.sales['total_sales']}\nTotal revenue: \u20B9{store.sales['total_revenue']} INR")
        return
    ...
    # Sort by ID
    for entry_id in sorted(data.keys(), key=lambda x: int(x)):
        entry = data[entry_id]
